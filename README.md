...
| フォトリフレクター (スライド扉) | スライド戸の開閉検知 | 起動時キャリブレーション |
| フォトリフレクター (O->I) | 一方通行ゲートの通過検知 | 起動時キャリブレーション |
...
### 6.5 Cloudflare Tunnel（.env で安全に運用）

1. `infra/docker/.env.example` をコピーして `infra/docker/.env` を作成する
2. `CLOUDFLARED_TUNNEL_TOKEN` に Cloudflare の実トークンを設定する
3. 次を実行する
...
これにより、トークンを compose や config ファイルへ直書きせずに tunnel を起動できます。
...
### 8.1 センサー単体判定
- 測距: 一定閾値以下で通過候補
- フォトリフレクター(スライド扉): 起動時基準との差分で扉動作候補
- フォトリフレクター(O->I): 起動時基準との差分で扉動作候補
- 光: 基準値との差分で在室候補
- 焦電: 閾値以上で在室候補を補強
...
- フォトリフレクター(スライド扉): I->O側の一方通行ゲートとして人数更新
- フォトリフレクター(O->I): O->I側の一方通行ゲートとして人数更新
